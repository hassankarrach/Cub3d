...
/*   Updated: 2024/11/18 07:12:13 by kait-baa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/parser.h"

t_wall_frame	*fill_frame_struct(char **map2d, int x, int y)
{
	t_wall_frame	*frame;
	...
	while (map2d[max_rows++])
		;
	max_rows--;
	if ((x + 1) < ((int)ft_strlen(map2d[0]) - 1) && map2d[y][x + 1] == '0')
		available_directions[count++] = RIGHT;
	if ((x - 1 >= 0) && map2d[y][x - 1] == '0')
		available_directions[count++] = LEFT;
	if ((y - 1) >= 0 && map2d[y - 1][x] == '0')
		available_directions[count++] = TOP;
	if ((y + 1) < max_rows && map2d[y + 1][x] == '0')
...
void	init_wallframes_struct(t_args *cub_args)
{
	...
	while (cub_args->map2d[y])
	{
		x = 0;
		while (cub_args->map2d[y][x])
		{
			if (cub_args->map2d[y][x] == 'F')
				cub_args->frames[frame_number++]
					= fill_frame_struct(cub_args->map2d, x, y);
			x++;
		}
		y++;
	}
...
bool	is_logic_frame(t_args *cub_args, int y, int x)
{
	...
	if (x - 1 >= 0)
		left_side = cub_args->map2d[y][x - 1];
	else
		left_side = 'k';
	if (x + 1 < cub_args->map_columns)
		right_side = cub_args->map2d[y][x + 1];
	else
		right_side = 'k';
	if (y + 1 < cub_args->map_rows)
		bottom_side = cub_args->map2d[y + 1][x];
	else
		bottom_side = 'k';
	if (y - 1 >= 0)
		top_side = cub_args->map2d[y - 1][x];
	else
		top_side = 'k';
